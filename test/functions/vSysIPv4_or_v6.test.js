...
const rIPV6Val = () => rIPV6ValidChars[Math.floor(Math.random() * 16)];
const rIPV6Chunk = () => `${rIPV6Val()}${rIPV6Val()}${rIPV6Val()}${rIPV6Val()}`;
...
describe("vSysIPv4orv6", () => {
    ...
    it ('[ipv4] Should be invalid when passed ipv4 addresses where a single octet goes beyond the upper bound of a byte (255)', () => {
        ...
        for (let i = 0; i < 500; i++) {
            expect(v.validate({a: `${rIPV4Correct()}.${rIPV4Incorrect()}.${rIPV4Correct()}.${rIPV4Correct()}`}).is_valid).to.eql(false);
        }

        for (let i = 0; i < 500; i++) {
            expect(v.validate({a: `${rIPV4Correct()}.${rIPV4Correct()}.${rIPV4Incorrect()}.${rIPV4Correct()}`}).is_valid).to.eql(false);
        }

        for (let i = 0; i < 500; i++) {
            expect(v.validate({a: `${rIPV4Correct()}.${rIPV4Correct()}.${rIPV4Correct()}.${rIPV4Incorrect()}`}).is_valid).to.eql(false);
        }
    });
    ...
    it ('[ipv4] Should be invalid when passed ipv4 addresses where all octets are correct but where the string contains a non-dot separator', () => {
        ...
        for (let i = 0; i < 46; i++) {
            ...
            expect(v.validate({a: `${rIPV4Correct()}${sep}${rIPV4Correct()}${sep}${rIPV4Correct()}${sep}${rIPV4Correct()}`}).is_valid).to.eql(false);
        }
    ...
    it ('[ipv6] Should be invalid when passed ipv6 addresses where all chunks are correct but where the string contains a non-dot separator', () => {
        ...
        for (let i = 0; i < 58; i++) {
            const sep = String.fromCharCode(i);
            expect(v.validate({a: `${rIPV6Chunk()}${sep}${rIPV6Chunk()}${sep}${rIPV6Chunk()}${sep}${rIPV6Chunk()}${sep}${rIPV6Chunk()}${sep}${rIPV6Chunk()}${sep}${rIPV6Chunk()}${sep}${rIPV6Chunk()}`}).is_valid).to.eql(false);
        }
