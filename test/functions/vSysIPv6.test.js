...
const rVal = () => validChars[Math.floor(Math.random() * 16)];

const rChunk = () => `${rVal()}${rVal()}${rVal()}${rVal()}`;
...
describe('vSysIPv6', () => {
    ...
    it('Should be invalid if not passed a string', () => {
        for (const el of str_tests) {
            expect(new Validator({a: 'sys_ipv6'}).validate({a: el}).is_valid).to.eql(false);
        }
    });
    ...
    it('Should be invalid when passed a ipv6 address containing only invalid characters and no separator', () => {
    ...
    it('Should be invalid when passed a ipv6 address containing only invalid characters and with separator', () => {
        const v = new Validator({a: 'sys_ipv6'});
    ...
    it('Should be invalid when passed a ipv6 address containing a single chunk with invalid characters and with separator', () => {
        const v = new Validator({a: 'sys_ipv6'});
        ...
        for (let i = 0; i < 48; i++) {
            expect(v.validate({a: tpl1(String.fromCharCode(i))}).is_valid).to.eql(false);
            expect(v.validate({a: tpl2(String.fromCharCode(i))}).is_valid).to.eql(false);
            expect(v.validate({a: tpl3(String.fromCharCode(i))}).is_valid).to.eql(false);
            expect(v.validate({a: tpl4(String.fromCharCode(i))}).is_valid).to.eql(false);
            expect(v.validate({a: tpl5(String.fromCharCode(i))}).is_valid).to.eql(false);
            expect(v.validate({a: tpl6(String.fromCharCode(i))}).is_valid).to.eql(false);
            expect(v.validate({a: tpl7(String.fromCharCode(i))}).is_valid).to.eql(false);
            expect(v.validate({a: tpl8(String.fromCharCode(i))}).is_valid).to.eql(false);
        }
    ...
    it('Should be invalid when passed a ipv6 address containing a single invalid character and with separator', () => {
        const v = new Validator({a: 'sys_ipv6'});
        ...
        for (let i = 0; i < 48; i++) {
            ...
            expect(v.validate({a: tpl7(String.fromCharCode(i))}).is_valid).to.eql(false);
            expect(v.validate({a: tpl8(String.fromCharCode(i))}).is_valid).to.eql(false);
            expect(v.validate({a: tpl9(String.fromCharCode(i))}).is_valid).to.eql(false);
            expect(v.validate({a: tpl10(String.fromCharCode(i))}).is_valid).to.eql(false);
            expect(v.validate({a: tpl11(String.fromCharCode(i))}).is_valid).to.eql(false);
            expect(v.validate({a: tpl12(String.fromCharCode(i))}).is_valid).to.eql(false);
            expect(v.validate({a: tpl13(String.fromCharCode(i))}).is_valid).to.eql(false);
            expect(v.validate({a: tpl14(String.fromCharCode(i))}).is_valid).to.eql(false);
            expect(v.validate({a: tpl15(String.fromCharCode(i))}).is_valid).to.eql(false);
            expect(v.validate({a: tpl16(String.fromCharCode(i))}).is_valid).to.eql(false);
            expect(v.validate({a: tpl17(String.fromCharCode(i))}).is_valid).to.eql(false);
            expect(v.validate({a: tpl18(String.fromCharCode(i))}).is_valid).to.eql(false);
            expect(v.validate({a: tpl19(String.fromCharCode(i))}).is_valid).to.eql(false);
            expect(v.validate({a: tpl20(String.fromCharCode(i))}).is_valid).to.eql(false);
            expect(v.validate({a: tpl21(String.fromCharCode(i))}).is_valid).to.eql(false);
            expect(v.validate({a: tpl22(String.fromCharCode(i))}).is_valid).to.eql(false);
            expect(v.validate({a: tpl23(String.fromCharCode(i))}).is_valid).to.eql(false);
            expect(v.validate({a: tpl24(String.fromCharCode(i))}).is_valid).to.eql(false);
            expect(v.validate({a: tpl25(String.fromCharCode(i))}).is_valid).to.eql(false);
            expect(v.validate({a: tpl26(String.fromCharCode(i))}).is_valid).to.eql(false);
            expect(v.validate({a: tpl27(String.fromCharCode(i))}).is_valid).to.eql(false);
            expect(v.validate({a: tpl28(String.fromCharCode(i))}).is_valid).to.eql(false);
            expect(v.validate({a: tpl29(String.fromCharCode(i))}).is_valid).to.eql(false);
            expect(v.validate({a: tpl30(String.fromCharCode(i))}).is_valid).to.eql(false);
            expect(v.validate({a: tpl31(String.fromCharCode(i))}).is_valid).to.eql(false);
            expect(v.validate({a: tpl32(String.fromCharCode(i))}).is_valid).to.eql(false);
        }
    ...
    it('Should be invalid when passed ipv6 addresses with only 1 chunk', () => {
        const v = new Validator({a: 'sys_ipv6'});
        for (let i = 0; i < 500; i++) {
            expect(v.validate({a: `${rChunk()}`}).is_valid).to.eql(false);
        }
    });

    it('Should be invalid when passed ipv6 addresses with only 2 chunks', () => {
        const v = new Validator({a: 'sys_ipv6'});
        for (let i = 0; i < 500; i++) {
            expect(v.validate({a: `${rChunk()}:${rChunk()}`}).is_valid).to.eql(false);
        }
    });

    it('Should be invalid when passed ipv6 addresses with only 3 chunks', () => {
        const v = new Validator({a: 'sys_ipv6'});
        for (let i = 0; i < 500; i++) {
            expect(v.validate({a: `${rChunk()}:${rChunk()}:${rChunk()}`}).is_valid).to.eql(false);
        }
    });

    it('Should be invalid when passed ipv6 addresses with only 4 chunks', () => {
        const v = new Validator({a: 'sys_ipv6'});
        for (let i = 0; i < 500; i++) {
            expect(v.validate({a: `${rChunk()}:${rChunk()}:${rChunk()}:${rChunk()}`}).is_valid).to.eql(false);
        }
    });

    it('Should be invalid when passed ipv6 addresses with only 5 chunks', () => {
        const v = new Validator({a: 'sys_ipv6'});
        for (let i = 0; i < 500; i++) {
            expect(v.validate({a: `${rChunk()}:${rChunk()}:${rChunk()}:${rChunk()}:${rChunk()}`}).is_valid).to.eql(false);
        }
    });

    it('Should be invalid when passed ipv6 addresses with only 6 chunks', () => {
        const v = new Validator({a: 'sys_ipv6'});
        for (let i = 0; i < 500; i++) {
            expect(v.validate({a: `${rChunk()}:${rChunk()}:${rChunk()}:${rChunk()}:${rChunk()}:${rChunk()}`}).is_valid).to.eql(false);
        }
    });

    it('Should be invalid when passed ipv6 addresses with only 7 chunks', () => {
        const v = new Validator({a: 'sys_ipv6'});
        for (let i = 0; i < 500; i++) {
            expect(v.validate({a: `${rChunk()}:${rChunk()}:${rChunk()}:${rChunk()}:${rChunk()}:${rChunk()}:${rChunk()}`}).is_valid).to.eql(false);
        }
    });

    it('Should be invalid when passed ipv6 addresses where all chunks are correct but where the string contains spaces', () => {
        const v = new Validator({a: 'sys_ipv6'});
        for (let i = 0; i < 500; i++) {
            expect(v.validate({a: `  ${rChunk()}.${rChunk()}.${rChunk()}.${rChunk()}${rChunk()}.${rChunk()}.${rChunk()}.${rChunk()}`}).is_valid).to.eql(false);
            expect(v.validate({a: `${rChunk()}.${rChunk()}.${rChunk()}.${rChunk()}${rChunk()}.${rChunk()}.${rChunk()}.${rChunk()}   `}).is_valid).to.eql(false);
            expect(v.validate({a: `  ${rChunk()}.${rChunk()}.${rChunk()}.${rChunk()}${rChunk()}.${rChunk()}.${rChunk()}.${rChunk()}    `}).is_valid).to.eql(false);
        }
    });

    it('Should be invalid when passed ipv6 addresses where all chunks are correct but where the string contains a non-dot separator', () => {
        const v = new Validator({a: 'sys_ipv6'});
        ...
        for (let i = 0; i < 58; i++) {
            ...
            expect(v.validate({a: `${rChunk()}${sep}${rChunk()}${sep}${rChunk()}${sep}${rChunk()}${sep}${rChunk()}${sep}${rChunk()}${sep}${rChunk()}${sep}${rChunk()}`}).is_valid).to.eql(false);
        }
    ...
    it('Should be valid when passed a list of valid ipv6 addresses (sample list)', () => {
        const lst = [
    ...
    it('Should be valid when passing ipv6 addresses in shortened format (sample list)', () => {
        const lst = [
