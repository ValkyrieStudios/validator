...
describe("vGreaterThanOrEqual", () => {
    ...
    it ('should be invalid when no params are passed', () => {
        ...
        expect(evaluation.errors.a).to.deep.equal([{msg: 'greater_than_or_equal', params: [undefined] }]);
    });

    it ('should return a correct error message when invalid', () => {
        const evaluation = (new Validator({ a: 'greater_than_or_equal:10' })).validate({ a: 5 });
        expect(evaluation.is_valid).to.eql(false);
        expect(evaluation.errors.a).to.deep.equal([{msg: 'greater_than_or_equal', params: ['10']}]);
    });
    ...
    it ('should not allow other types as values', () => {
        ...
        invalid_keys.forEach(key => expect(evaluation.errors[key]).to.deep.equal([{msg:'greater_than_or_equal', params: ['2']}]));
    });
