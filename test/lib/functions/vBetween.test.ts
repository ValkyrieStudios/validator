...
describe('vBetween', () => {
    ...
    it('Should be invalid when incorrect params are passed as part of string literal', () => {
        for (const el of ['hello', true, false]) {
            ...
            assert.deepEqual(evaluation, {is_valid: false, count: 1,errors: {a: [{msg: 'between', params: ['0', `${el}`]}]}});
        }

        for (const el of ['hello', true, false]) {
            const evaluation = new Validator({a: `between:${el},0`}).validate({a: 10});
            assert.deepEqual(evaluation, {is_valid: false, count: 1,errors: {a: [{msg: 'between', params: [`${el}`, '0']}]}});
        }

        for (const el of ['hello', true, false]) {
            const evaluation = new Validator({a: `between:${el},${el}`}).validate({a: 10});
            assert.deepEqual(evaluation, {is_valid: false, count: 1,errors: {a: [{msg: 'between', params: [`${el}`, `${el}`]}]}});
        }

        const evaluation3 = new Validator({a: 'between:0'}).validate({a: 10});
        assert.deepEqual(evaluation3, {is_valid: false, count: 1,errors: {a: [{msg: 'between', params: ['0']}]}});

        const evaluation5 = new Validator({a: 'between'}).validate({a: 10});
        assert.deepEqual(evaluation5, {is_valid: false, count: 1,errors: {a: [{msg: 'between', params: []}]}});
    });
    ...
    describe('Array', () => {
        ...
        it('Should validate an array whose length is equal to the start parameter as invalid', () => {
            ...
            assert.deepEqual(evaluation, {is_valid: false, count: 1, errors: {a: [{msg: 'between', params: ['3', '10']}]}});
        });
        ...
        it('Should validate an array whose length is equal to the end parameter as invalid', () => {
            ...
            assert.deepEqual(evaluation, {is_valid: false, count: 1, errors: {a: [{msg: 'between', params: ['1', '3']}]}});
        });
    ...
    describe('Blob and File', () => {
        it('Should validate a Blob whose size is between the provided parameters as valid', () => {
            ...
            assert.ok(new Validator({a: 'between:10,100'}).check({a: mediumBlob}));
        });
        ...
        it('Should validate a Blob whose size is equal to the start parameter as invalid', () => {
            ...
            assert.deepEqual(new Validator({a: 'between:31,100'}).validate({a: exactBlob}), {
                is_valid: false,
                count: 1,
                errors: {
        ...
        it('Should validate a Blob whose size is equal to the end parameter as invalid', () => {
            ...
            assert.deepEqual(new Validator({a: 'between:10,100'}).validate({a: exactBlob}), {
                is_valid: false,
                count: 1,
                errors: {
        ...
        it('Should validate a Blob whose size is smaller than the start parameter as invalid', () => {
            ...
            assert.deepEqual(new Validator({a: 'between:10,100'}).validate({a: smallBlob}), {
                is_valid: false,
                count: 1,
                errors: {
                    a: [{msg: 'between', params: ['10', '100']}],
        ...
        it('Should validate a Blob whose size is larger than the end parameter as invalid', () => {
            ...
            assert.deepEqual(new Validator({a: 'between:10,50'}).validate({a: largeBlob}), {
                is_valid: false,
                count: 1,
                errors: {
        ...
        it('Should validate a File whose size is between the provided parameters as valid', () => {
            ...
            assert.ok(new Validator({a: 'between:10,100'}).check({a: mediumFile}));
        });
        ...
        it('Should validate a File whose size is equal to the start parameter as invalid', () => {
            ...
            assert.deepEqual(new Validator({a: 'between:25,100'}).validate({a: exactFile}), {
                is_valid: false,
                count: 1,
                errors: {
        ...
        it('Should validate a File whose size is equal to the end parameter as invalid', () => {
            ...
            assert.deepEqual(new Validator({a: 'between:10,100'}).validate({a: exactFile}), {
                is_valid: false,
                count: 1,
                errors: {
                    a: [{msg: 'between', params: ['10', '100']}],
        ...
        it('Should validate a File whose size is smaller than the start parameter as invalid', () => {
            ...
            assert.deepEqual(new Validator({a: 'between:20,50'}).validate({a: smallFile}), {
                is_valid: false,
                count: 1,
                errors: {
        ...
        it('Should validate a File whose size is larger than the end parameter as invalid', () => {
            ...
            assert.deepEqual(new Validator({a: 'between:10,30'}).validate({a: largeFile}), {
                is_valid: false,
                count: 1,
                errors: {
    ...
    describe('Number', () => {
        ...
        it('Should validate a number equal to the provided start parameter as invalid', () => {
            ...
            assert.deepEqual(evaluation, {is_valid: false, count: 1, errors: {a: [{msg: 'between', params: ['42', '100']}]}});
        });
        ...
        it('Should validate a number equal to the provided end parameter as invalid', () => {
            ...
            assert.deepEqual(evaluation, {is_valid: false, count: 1, errors: {a: [{msg: 'between', params: ['1', '42']}]}});
        });
        ...
        it('Should validate a number smaller than the provided start parameter as invalid', () => {
            ...
            assert.deepEqual(evaluation, {is_valid: false, count: 1, errors: {a: [{msg: 'between', params: ['100', '200']}]}});
        });
        ...
        it('Should validate a number larger than the provided end parameter as invalid', () => {
            ...
            assert.deepEqual(evaluation, {is_valid: false, count: 1, errors: {a: [{msg: 'between', params: ['10', '25']}]}});
        });
        ...
        it('Should validate negative number ranges that are below the provided start parameter as invalid', () => {
            ...
            assert.deepEqual(evaluation, {is_valid: false, count: 1, errors: {a: [{msg: 'between', params: ['-60', '-50']}]}});
        });
        ...
        it('Should validate negative number ranges that are above the provided parameter as invalid', () => {
            ...
            assert.deepEqual(evaluation, {is_valid: false, count: 1, errors: {a: [{msg: 'between', params: ['-40', '-20']}]}});
        });
        ...
        it('Should validate negative number ranges that are equal to the provided start parameter as invalid', () => {
            ...
            assert.deepEqual(evaluation, {is_valid: false, count: 1, errors: {a: [{msg: 'between', params: ['-999', '-100']}]}});
        });
        ...
        it('Should validate negative number ranges that are equal to the provided end parameter as invalid', () => {
            ...
            assert.deepEqual(evaluation, {is_valid: false, count: 1, errors: {a: [{msg: 'between', params: ['-10000', '-999']}]}});
        });
    });
});
