...
describe('vUrlVideo', () => {
    it('Should validate a url string that is a valid url and a video url as valid', () => {
        const v = new Validator({a: 'url_vid'});
    ...
    it('Should validate a url string that is a valid media url but not a video url as invalid', () => {
        const v = new Validator({a: 'url_vid'});

        for (const ext of [...AUDIO_EXTENSIONS, ...IMAGE_EXTENSIONS].filter(el => el !== 'ogg')) {
            for (const el of [
                `http://foo.com/blah_blah/${guid()}.${ext}`,
                `http://foo.com/blah_blah_(wikipedia).${ext}`,
                `http://foo.com/blah_blah_(wikipedia)_(again)/${guid()}.${ext}`,
    ...
    it('Should not validate a url string that is a valid url but not a video url as valid', () => {
        const v = new Validator({a: 'url_vid'});
    ...
    it('Should not validate other types as valid urls', () => {
        ...
        for (const el of CONSTANTS.NOT_STRING_WITH_EMPTY) {
            ...
            assert.deepEqual(evaluation, {
                ...
                count: 1,
                errors: {
                    a: [{msg: el === undefined ? 'not_found' : 'url_vid', params: []}],
                },
            });
        }
    });

    it('Should not validate strings that are video urls after trimming as valid', () => {
        const v = new Validator({a: 'url_vid'});
