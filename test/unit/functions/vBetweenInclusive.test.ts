...
describe('vBetweenInclusive', () => {
    ...
    it('Should be invalid when incorrect params are passed as part of string literal', () => {
        for (const el of ['hello', true, false]) {
            ...
            expect(evaluation).toEqual({is_valid: false, count: 1,errors: {a: [{msg: 'between_inc', params: ['0', `${el}`]}]}});
        }

        for (const el of ['hello', true, false]) {
            const evaluation = new Validator({a: `between_inc:${el},0`}).validate({a: 10});
            expect(evaluation).toEqual({is_valid: false, count: 1,errors: {a: [{msg: 'between_inc', params: [`${el}`, '0']}]}});
        }

        for (const el of ['hello', true, false]) {
            const evaluation = new Validator({a: `between_inc:${el},${el}`}).validate({a: 10});
            expect(evaluation).toEqual({is_valid: false, count: 1,errors: {a: [{msg: 'between_inc', params: [`${el}`, `${el}`]}]}});
        }

        const evaluation3 = new Validator({a: 'between_inc:0'}).validate({a: 10});
        expect(evaluation3).toEqual({is_valid: false, count: 1,errors: {a: [{msg: 'between_inc', params: ['0']}]}});

        const evaluation5 = new Validator({a: 'between_inc'}).validate({a: 10});
        expect(evaluation5).toEqual({is_valid: false, count: 1,errors: {a: [{msg: 'between_inc', params: []}]}});
    });
    ...
    it('Should not allow other types as values', () => {
        ...
        invalid_keys.forEach(key => expect(evaluation.errors[key]).toEqual([{msg:'between_inc', params: ['2', '300']}]));
    });
    ...
    describe('String', () => {
        ...
        it('Should validate a string whose length is smaller than the start parameter as invalid', () => {
            ...
            expect(evaluation).toEqual({is_valid: false, count: 1, errors: {a: [{msg: 'between_inc', params: ['10', '20']}]}});
        });
        ...
        it('Should validate a string whose length is larger than the end parameter as invalid', () => {
            ...
            expect(evaluation).toEqual({is_valid: false, count: 1, errors: {a: [{msg: 'between_inc', params: ['2', '5']}]}});
        });
    });
    ...
    describe('Blob and File', () => {
        it('Should validate a Blob whose size is between the provided parameters as valid', () => {
            ...
            expect(new Validator({a: 'between_inc:10,100'}).check({a: mediumBlob})).toBe(true);
        });
        ...
        it('Should validate a Blob whose size is equal to the start parameter as valid', () => {
            ...
            expect(new Validator({a: 'between_inc:27,100'}).check({a: exactBlob})).toBe(true);
        });
        ...
        it('Should validate a Blob whose size is equal to the end parameter as valid', () => {
            ...
            expect(new Validator({a: 'between_inc:10,100'}).check({a: exactBlob})).toBe(true);
        });

        it('Should validate a Blob whose size is smaller than the start parameter as invalid', () => {
            const smallBlob = new Blob(['Short']);
            expect(new Validator({a: 'between_inc:10,100'}).validate({a: smallBlob})).toEqual({
                is_valid: false,
                count: 1,
                errors: {
        ...
        it('Should validate a Blob whose size is larger than the end parameter as invalid', () => {
            ...
            expect(new Validator({a: 'between_inc:10,50'}).validate({a: largeBlob})).toEqual({
                is_valid: false,
                count: 1,
                errors: {
        ...
        it('Should validate a File whose size is between the provided parameters as valid', () => {
            ...
            expect(new Validator({a: 'between_inc:10,100'}).check({a: mediumFile})).toBe(true);
        });
        ...
        it('Should validate a File whose size is equal to the start parameter as valid', () => {
            ...
            expect(new Validator({a: 'between_inc:16,100'}).check({a: exactFile})).toBe(true);
        });
        ...
        it('Should validate a File whose size is equal to the end parameter as valid', () => {
            ...
            expect(new Validator({a: 'between_inc:10,100'}).check({a: exactFile})).toBe(true);
        });
        ...
        it('Should validate a File whose size is smaller than the start parameter as invalid', () => {
            ...
            expect(new Validator({a: 'between_inc:20,50'}).validate({a: smallFile})).toEqual({
                is_valid: false,
                count: 1,
                errors: {
        ...
        it('Should validate a File whose size is larger than the end parameter as invalid', () => {
            ...
            expect(new Validator({a: 'between_inc:10,50'}).validate({a: largeFile})).toEqual({
                is_valid: false,
                count: 1,
                errors: {
                    a: [{msg: 'between_inc', params: ['10', '50']}],
    ...
    describe('Number', () => {
        ...
        it('Should validate a number smaller than the provided start parameter as invalid', () => {
            ...
            expect(evaluation).toEqual({is_valid: false, count: 1, errors: {a: [{msg: 'between_inc', params: ['100', '200']}]}});
        });
        ...
        it('Should validate a number larger than the provided end parameter as invalid', () => {
            ...
            expect(evaluation).toEqual({is_valid: false, count: 1, errors: {a: [{msg: 'between_inc', params: ['10', '25']}]}});
        });
        ...
        it('Should validate negative number ranges that are below the provided start parameter as invalid', () => {
            ...
            expect(evaluation).toEqual({is_valid: false, count: 1, errors: {a: [{msg: 'between_inc', params: ['-60', '-50']}]}});
        });
        ...
        it('Should validate negative number ranges that are above the provided parameter as invalid', () => {
            ...
            expect(evaluation).toEqual({is_valid: false, count: 1, errors: {a: [{msg: 'between_inc', params: ['-40', '-20']}]}});
        });
