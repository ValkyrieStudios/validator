...
describe('vMax', () => {
    ...
    it('Should allow a string as an acceptable value', () => {
        expect(new Validator({a: 'max:7'}).check({a: 'hello'})).toBe(true);
    });

    it('Should allow an array as an acceptable value', () => {
        expect(new Validator({a: 'max:10'}).check({a: ['foo', 'bar', 'bar', 'foo', 'hello', 'world']})).toBe(true);
    });

    it('Should allow a numerical value as an acceptable value', () => {
        expect(new Validator({a: 'max:20'}).check({a: 15})).toBe(true);
    });

    it('Should disallow NaN values', () => {
        expect(new Validator({a: 'max:10'}).validate({a: Number.NaN})).toEqual({
            is_valid: false,
            count: 1,
            errors: {
                a: [{msg: 'max', params: ['10']}],
    ...
    describe('Array', () => {
        it('Should validate an array whose length is smaller than the provided parameter as valid', () => {
            expect(new Validator({a: 'max:10'}).check({a: [1,2,3]})).toBe(true);
        });

        it('Should validate an array whose length is equal to the provided parameter as valid', () => {
            expect(new Validator({a: 'max:6'}).check({a: [1,2,3,4,5,6]})).toBe(true);
        });

        it('Should validate an array whose length is larger than the provided parameter as invalid', () => {
            expect(new Validator({a: 'max:4'}).validate({a: ['apple', 'fear', 'mango', 'grape', 'orange']})).toEqual({
                is_valid: false,
                count: 1,
                errors: {
                    a: [{msg: 'max', params: ['4']}],
    ...
    describe('Number', () => {
        it('Should validate a number smaller than the provided parameter as valid', () => {
            expect(new Validator({a: 'max:1000'}).check({a: 950})).toBe(true);
        });

        it('Should validate a number equal to the provided parameter as valid', () => {
            expect(new Validator({a: 'max:42'}).check({a: 42})).toBe(true);
        });

        it('Should validate a number larger than the provided parameter as invalid', () => {
            expect(new Validator({a: 'max:9000'}).validate({a: 9999})).toEqual({
                is_valid: false,
                count: 1,
                errors: {
        ...
        it('Should validate negative number ranges that are below the provided parameter as valid', () => {
            expect(new Validator({a: 'max:-500'}).check({a: -501})).toBe(true);
        });

        it('Should validate negative number ranges that are above the provided parameter as invalid', () => {
            expect(new Validator({a: 'max:-20'}).validate({a: -18})).toEqual({
                is_valid: false,
                count: 1,
                errors: {
                    a: [{msg: 'max', params: ['-20']}],
        ...
        it('Should validate negative number ranges that are equal to the provided parameter as valid', () => {
            expect(new Validator({a: 'max:-999'}).check({a: -999})).toBe(true);
        });
    });
});
