...
describe('vSysIPv6', () => {
    it('Should be invalid if not passed a string or passed an empty string after trimming', () => {
        for (const el of CONSTANTS.NOT_STRING_WITH_EMPTY) {
            expect(new Validator({a: 'sys_ipv6'}).validate({a: el})).toEqual({
                is_valid: false,
                count: 1,
                errors: {
    ...
    it('Should be invalid with address containing only invalid characters and no separator', () => {
        ...
        for (let i = 0; i < 48; i++) {
            expect(v.check({a: tpl(String.fromCharCode(i))})).toBe(false);
        }
    ...
    it('Should be invalid with address containing a single chunk with invalid characters and with separator', () => {
        ...
        for (let i = 0; i < 48; i++) {
            expect(v.check({a: tpl1(String.fromCharCode(i))})).toBe(false);
            expect(v.check({a: tpl2(String.fromCharCode(i))})).toBe(false);
            expect(v.check({a: tpl3(String.fromCharCode(i))})).toBe(false);
            expect(v.check({a: tpl4(String.fromCharCode(i))})).toBe(false);
            expect(v.check({a: tpl5(String.fromCharCode(i))})).toBe(false);
            expect(v.check({a: tpl6(String.fromCharCode(i))})).toBe(false);
            expect(v.check({a: tpl7(String.fromCharCode(i))})).toBe(false);
            expect(v.check({a: tpl8(String.fromCharCode(i))})).toBe(false);
        }
    ...
    it('Should be invalid with address containing a single invalid character and with separator', () => {
        ...
        for (let i = 0; i < 48; i++) {
            ...
            expect(v.check({a: tpl7(String.fromCharCode(i))})).toBe(false);
            expect(v.check({a: tpl8(String.fromCharCode(i))})).toBe(false);
            expect(v.check({a: tpl9(String.fromCharCode(i))})).toBe(false);
            expect(v.check({a: tpl10(String.fromCharCode(i))})).toBe(false);
            expect(v.check({a: tpl11(String.fromCharCode(i))})).toBe(false);
            expect(v.check({a: tpl12(String.fromCharCode(i))})).toBe(false);
            expect(v.check({a: tpl13(String.fromCharCode(i))})).toBe(false);
            expect(v.check({a: tpl14(String.fromCharCode(i))})).toBe(false);
            expect(v.check({a: tpl15(String.fromCharCode(i))})).toBe(false);
            expect(v.check({a: tpl16(String.fromCharCode(i))})).toBe(false);
            expect(v.check({a: tpl17(String.fromCharCode(i))})).toBe(false);
            expect(v.check({a: tpl18(String.fromCharCode(i))})).toBe(false);
            expect(v.check({a: tpl19(String.fromCharCode(i))})).toBe(false);
            expect(v.check({a: tpl20(String.fromCharCode(i))})).toBe(false);
            expect(v.check({a: tpl21(String.fromCharCode(i))})).toBe(false);
            expect(v.check({a: tpl22(String.fromCharCode(i))})).toBe(false);
            expect(v.check({a: tpl23(String.fromCharCode(i))})).toBe(false);
            expect(v.check({a: tpl24(String.fromCharCode(i))})).toBe(false);
            expect(v.check({a: tpl25(String.fromCharCode(i))})).toBe(false);
            expect(v.check({a: tpl26(String.fromCharCode(i))})).toBe(false);
            expect(v.check({a: tpl27(String.fromCharCode(i))})).toBe(false);
            expect(v.check({a: tpl28(String.fromCharCode(i))})).toBe(false);
            expect(v.check({a: tpl29(String.fromCharCode(i))})).toBe(false);
            expect(v.check({a: tpl30(String.fromCharCode(i))})).toBe(false);
            expect(v.check({a: tpl31(String.fromCharCode(i))})).toBe(false);
            expect(v.check({a: tpl32(String.fromCharCode(i))})).toBe(false);
        }
    ...
    it('Should be invalid with addresses with only 1 chunk', () => {
        const v = new Validator({a: 'sys_ipv6'});
        for (let i = 0; i < 500; i++) {
            expect(v.check({a: `${rC()}`})).toBe(false);
        }
    });
    ...
    it('Should be invalid with addresses with only 2 chunks', () => {
        const v = new Validator({a: 'sys_ipv6'});
        for (let i = 0; i < 500; i++) {
            expect(v.check({a: `${rC()}:${rC()}`})).toBe(false);
        }
    });
    ...
    it('Should be invalid with addresses with only 3 chunks', () => {
        const v = new Validator({a: 'sys_ipv6'});
        for (let i = 0; i < 500; i++) {
            expect(v.check({a: `${rC()}:${rC()}:${rC()}`})).toBe(false);
        }
    });
    ...
    it('Should be invalid with addresses with only 4 chunks', () => {
        const v = new Validator({a: 'sys_ipv6'});
        for (let i = 0; i < 500; i++) {
            expect(v.check({a: `${rC()}:${rC()}:${rC()}:${rC()}`})).toBe(false);
        }
    });
    ...
    it('Should be invalid with addresses with only 5 chunks', () => {
        const v = new Validator({a: 'sys_ipv6'});
        for (let i = 0; i < 500; i++) {
            expect(v.check({a: `${rC()}:${rC()}:${rC()}:${rC()}:${rC()}`})).toBe(false);
        }
    });
    ...
    it('Should be invalid with addresses with only 6 chunks', () => {
        const v = new Validator({a: 'sys_ipv6'});
        for (let i = 0; i < 500; i++) {
            expect(v.check({a: `${rC()}:${rC()}:${rC()}:${rC()}:${rC()}:${rC()}`})).toBe(false);
        }
    });
    ...
    it('Should be invalid with addresses with only 7 chunks', () => {
        const v = new Validator({a: 'sys_ipv6'});
        for (let i = 0; i < 500; i++) {
            expect(v.check({a: `${rC()}:${rC()}:${rC()}:${rC()}:${rC()}:${rC()}:${rC()}`})).toBe(false);
        }
    });
    ...
    it('Should be invalid with addresses where all chunks are correct but where the string contains spaces', () => {
        const v = new Validator({a: 'sys_ipv6'});
        for (let i = 0; i < 500; i++) {
            expect(v.check({a: `  ${rC()}.${rC()}.${rC()}.${rC()}${rC()}.${rC()}.${rC()}.${rC()}`})).toBe(false);
            expect(v.check({a: `${rC()}.${rC()}.${rC()}.${rC()}${rC()}.${rC()}.${rC()}.${rC()}   `})).toBe(false);
            expect(v.check({a: `  ${rC()}.${rC()}.${rC()}.${rC()}${rC()}.${rC()}.${rC()}.${rC()}    `})).toBe(false);
        }
    });
    ...
    it('Should be invalid with addresses where all chunks are correct but where the string contains a non-dot separator', () => {
        ...
        for (let i = 0; i < 58; i++) {
            ...
            expect(v.check({a: [rC(), s, rC(), s, rC(), s, rC(), s, rC(), s, rC(), s, rC(), s, rC()].join('')})).toBe(false);
        }
