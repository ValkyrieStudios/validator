...
describe('vSysIPv4', () => {
    it('Should be invalid if not passed a string or passed an empty string after trimming', () => {
        for (const el of CONSTANTS.NOT_STRING_WITH_EMPTY) {
            expect(new Validator({a: 'sys_ipv4'}).validate({a: el})).toEqual({
                is_valid: false,
                count: 1,
                errors: {
    ...
    it('Should be invalid with address containing a single octet with non-numeric characters', () => {
        ...
        for (let i = 0; i < 48; i++) {
            expect(v.validate({a: tplOct1(String.fromCharCode(i))})).toEqual({
                is_valid: false,
                count: 1,
                errors: {a: [{msg: 'sys_ipv4', params: []}]},
            });
            expect(v.validate({a: tplOct2(String.fromCharCode(i))})).toEqual({
                is_valid: false,
                count: 1,
                errors: {a: [{msg: 'sys_ipv4', params: []}]},
            });
            expect(v.validate({a: tplOct3(String.fromCharCode(i))})).toEqual({
                is_valid: false,
                count: 1,
                errors: {a: [{msg: 'sys_ipv4', params: []}]},
            });
            expect(v.validate({a: tplOct4(String.fromCharCode(i))})).toEqual({
                is_valid: false,
                count: 1,
                errors: {a: [{msg: 'sys_ipv4', params: []}]},
            });
    ...
    it('Should be invalid with addresses with only 1 octet', () => {
        const v = new Validator({a: 'sys_ipv4'});
        for (let i = 0; i < 500; i++) {
            expect(v.validate({a: `${rCorrect()}`})).toEqual({
                is_valid: false,
                count: 1,
                errors: {a: [{msg: 'sys_ipv4', params: []}]},
            });
    ...
    it('Should be invalid with addresses with only 2 octets', () => {
        const v = new Validator({a: 'sys_ipv4'});
        for (let i = 0; i < 500; i++) {
            expect(v.validate({a: [rCorrect(), rCorrect()].join('.')})).toEqual({
                is_valid: false,
                count: 1,
                errors: {a: [{msg: 'sys_ipv4', params: []}]},
            });
    ...
    it('Should be invalid with addresses with only 3 octets', () => {
        const v = new Validator({a: 'sys_ipv4'});
        for (let i = 0; i < 500; i++) {
            expect(v.validate({a: [rCorrect(), rCorrect(), rCorrect()].join('.')})).toEqual({
                is_valid: false,
                count: 1,
                errors: {a: [{msg: 'sys_ipv4', params: []}]},
            });
    ...
    it('Should be invalid with addresses where a single octet goes beyond the upper bound of a byte (255)', () => {
        const v = new Validator({a: 'sys_ipv4'});
        for (let i = 0; i < 500; i++) {
            expect(v.validate({a: [rIncorrect(), rCorrect(), rCorrect(), rCorrect()].join('.')})).toEqual({
                is_valid: false,
                count: 1,
                errors: {a: [{msg: 'sys_ipv4', params: []}]},
            });
        ...
        for (let i = 0; i < 500; i++) {
            expect(v.validate({a: [rCorrect(), rIncorrect(), rCorrect(), rCorrect()].join('.')})).toEqual({
                is_valid: false,
                count: 1,
                errors: {a: [{msg: 'sys_ipv4', params: []}]},
            });
        ...
        for (let i = 0; i < 500; i++) {
            expect(v.validate({a: [rCorrect(), rCorrect(), rIncorrect(), rCorrect()].join('.')})).toEqual({
                is_valid: false,
                count: 1,
                errors: {a: [{msg: 'sys_ipv4', params: []}]},
            });
        ...
        for (let i = 0; i < 500; i++) {
            expect(v.validate({a: [rCorrect(), rCorrect(), rCorrect(), rIncorrect()].join('.')})).toEqual({
                is_valid: false,
                count: 1,
                errors: {a: [{msg: 'sys_ipv4', params: []}]},
            });
    ...
    it('Should be invalid with addresses where all octets go beyond the upper bound of a byte (255)', () => {
        const v = new Validator({a: 'sys_ipv4'});
        for (let i = 0; i < 500; i++) {
            expect(v.validate({a: [rIncorrect(), rIncorrect(), rIncorrect(), rIncorrect()].join('.')})).toEqual({
                is_valid: false,
                count: 1,
                errors: {a: [{msg: 'sys_ipv4', params: []}]},
            });
    ...
    it('Should be invalid with addresses where all octets are correct but where the string contains spaces', () => {
        const v = new Validator({a: 'sys_ipv4'});
        for (let i = 0; i < 500; i++) {
            expect(v.validate({a: `  ${rCorrect()}.${rCorrect()}.${rCorrect()}.${rCorrect()}`})).toEqual({
                is_valid: false,
                count: 1,
                errors: {a: [{msg: 'sys_ipv4', params: []}]},
            });
    ...
    it('Should be invalid with addresses where all octets are correct but where the string contains a non-dot separator', () => {
        ...
        for (let i = 0; i < 46; i++) {
            ...
            expect(v.validate({a: [rCorrect(), s, rCorrect(), s, rCorrect(), s, rCorrect()].join('')})).toEqual({
                is_valid: false,
                count: 1,
                errors: {a: [{msg: 'sys_ipv4', params: []}]},
            });
