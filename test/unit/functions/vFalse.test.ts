...
describe('vFalse', () => {
    it('Should be invalid if not passed a boolean', () => {
        for (const el of CONSTANTS.NOT_BOOLEAN) {
            expect(new Validator({a: 'false'}).validate({a: el})).toEqual({
                is_valid: false,
                count: 1,
                errors: {
    ...
    it('Should be invalid if passed true', () => {
        expect(new Validator({a: 'false'}).validate({a: true})).toEqual({
            is_valid: false,
            count: 1,
            errors: {a: [{msg: 'false', params: []}]},
        });
    });

    it('Should be valid if passed false', () => {
        expect(new Validator({a: 'false'}).validate({a: false})).toEqual({
            is_valid: true,
            count: 0,
            errors: {},
        });
