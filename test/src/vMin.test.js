...
describe("vMin", () => {
    ...
    it ('should be invalid when no params are passed', () => {
        ...
        expect(evaluation.errors.a).toEqual([{msg: 'min', params: [undefined] }]);
    });

    it ('should return a correct error message when invalid', () => {
        const evaluation = (new Validator({ a: 'min:10' })).validate({ a: 5 });
        expect(evaluation.is_valid).toEqual(false);
        expect(evaluation.errors.a).toEqual([{msg: 'min', params: ['10']}]);
    });
    ...
    it ('should not allow other types as values', () => {
        ...
        invalid_keys.forEach(key => expect(evaluation.errors[key]).toEqual([{msg:'min', params: ['2']}]));
    });
