...
describe("vMax", () => {
    ...
    it ('should be invalid when no params are passed', () => {
        ...
        expect(evaluation.errors.a).toEqual([{msg: 'max', params: [undefined] }]);
    });

    it ('should return a correct error message when invalid', () => {
        const evaluation = (new Validator({ a: 'max:10' })).validate({ a: 11 });
        expect(evaluation.is_valid).toEqual(false);
        expect(evaluation.errors.a).toEqual([{msg: 'max', params: ['10']}]);
    });
    ...
    it ('should not allow other types as values', () => {
        ...
        invalid_keys.forEach(key => expect(evaluation.errors[key]).toEqual([{msg:'max', params: ['250']}]));
    });
