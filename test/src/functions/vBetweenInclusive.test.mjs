...
describe('vBetweenInclusive', () => {
    it('Should be invalid when no params are passed', () => {
        ...
        assert.deepEqual(evaluation.errors.a, [{msg: 'between_inc', params: [undefined, undefined]}]);
        ...
        assert.deepEqual(evaluation2.errors.a, [{msg: 'between_inc', params: [undefined, undefined]}]);
        ...
        assert.deepEqual(evaluation3.errors.a, [{msg: 'between_inc', params: [undefined, undefined]}]);
    });
    ...
    it('Should be invalid when params are passed but not all are numeric', () => {
        for (const el of ['hello', true, false]) {
            ...
            assert.equal(evaluation.is_valid, false);
            assert.deepEqual(evaluation.errors.a, [{msg: 'between_inc', params: [el, 20]}]);
        }

        for (const el of ['hello', true, false]) {
            const evaluation = new Validator({a: 'between_inc:<min>,<max>'}).validate({a: 10, min: 0, max: el});
            assert.equal(evaluation.is_valid, false);
            assert.deepEqual(evaluation.errors.a, [{msg: 'between_inc', params: [0, el]}]);
        }

        for (const el of ['hello', true, false]) {
            const evaluation = new Validator({a: 'between_inc:<min>,<max>'}).validate({a: 10, min: el, max: el});
            assert.equal(evaluation.is_valid, false);
            assert.deepEqual(evaluation.errors.a, [{msg: 'between_inc', params: [el, el]}]);
        }
    });
    ...
    it('Should be invalid when incorrect params are passed as part of string literal', () => {
        ...
        for (const el of ['hello', true, false]) {
            ...
            assert.equal(evaluation.is_valid, false);
            assert.deepEqual(evaluation.errors.a, [{msg: 'between_inc', params: [`${el}`, '0']}]);
        }
        ...
        assert.equal(evaluation3.is_valid, false);
        assert.deepEqual(evaluation3.errors.a, [{msg: 'between_inc', params: ['0']}]);

        const evaluation5 = new Validator({a: 'between_inc'}).validate({a: 10});
        assert.equal(evaluation5.is_valid, false);
        assert.deepEqual(evaluation5.errors.a, [{msg: 'between_inc', params: []}]);
    });

    it('Should return a correct error message when invalid', () => {
        const evaluation = new Validator({a: 'between_inc:5,15'}).validate({a: 25});
        assert.equal(evaluation.is_valid, false);
        assert.deepEqual(evaluation.errors.a, [{msg: 'between_inc', params: ['5', '15']}]);
    });
