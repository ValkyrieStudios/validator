...
describe('vGreaterThanOrEqual', () => {
    it('Should be invalid when no params are passed', () => {
        ...
        assert.deepEqual(evaluation.errors.a, [{msg: 'greater_than_or_equal', params: [undefined]}]);
    });
    ...
    it('Should be invalid when non-numeric params are passed as part of params', () => {
        for (const el of ['hello', true, false]) {
            ...
            assert.equal(evaluation.is_valid, false);
            assert.deepEqual(evaluation.errors.a, [{msg: 'greater_than_or_equal', params: [el]}]);
        }
    });
    ...
    it('Should return a correct error message when invalid', () => {
        ...
        assert.equal(evaluation.is_valid, false);
        assert.deepEqual(evaluation.errors.a, [{msg: 'greater_than_or_equal', params: ['10']}]);
    });
    ...
    it('Should not allow other types as values', () => {
        ...
        invalid_keys.forEach(key => assert.deepEqual(evaluation.errors[key], [{msg:'greater_than_or_equal', params: ['2']}]));
    });
