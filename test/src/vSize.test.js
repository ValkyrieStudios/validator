...
describe("vSize", () => {
    ...
    it ('should be invalid when no params are passed', () => {
        ...
        expect(evaluation.errors.a).toEqual([{msg: 'size', params: [undefined] }]);
    });

    it ('should return a correct error message when invalid', () => {
        const evaluation = (new Validator({ a: 'size:10' })).validate({ a: 'this is a string' });
        expect(evaluation.is_valid).toEqual(false);
        expect(evaluation.errors.a).toEqual([{msg: 'size', params: ['10']}]);
    });
    ...
    it ('should not allow other types as values', () => {
        ...
        invalid_keys.forEach(key => expect(evaluation.errors[key]).toEqual([{msg:'size', params: ['5']}]));
    });
