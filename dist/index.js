...
var Validator = /*#__PURE__*/function () {
    ...
    function parse(acc, key) {
      ...
      if (_is["default"].Object(cursor)) {
        ...
        (0, _set["default"])(acc, key, cursor.substring(startix).split('|').reduce(function (rule_acc, rule_string) {
          ...
          var type = params.shift().trim(); //  Get 'not' flag

          var not = type.startsWith('!');
          if (not) type = type.replace(/!/g, ''); //  Get parameters
  ...
  _createClass(Validator, [{
    ...
    value: function validate(data) {
      ...
      if (!data) {
        ...
        var run = function run(key) {
          ...
          var val = (0, _get["default"])(data, key); //  Iterable error flags

          var iterable_unique = true;
          var iterable_err = false;
          var iterable_min_err = false;
          var iterable_max_err = false; //  Validate array of rules for this property
          ...
          try {
            for (_iterator.s(); !(_step = _iterator.n()).done;) {
              ...
              if (!validateFn[rule.type]) throw new Error("Rule: ".concat(rule.type, " was not found")); //  Each param rule is a cb function that should be executed on each run, retrieving
              ...
              if (_is["default"].Object(rule.iterable)) {
                ...
                for (var i = 0; i < val.length; i++) {
                  ...
                  //  Run validation
                  var rule_valid = (_validateFn$rule$type = validateFn[rule.type]).call.apply(_validateFn$rule$type, [_this, val[i]].concat(params)); //  If check fails (not valid && not not | not && valid)
                ...
                //  Run validation
                var _rule_valid = (_validateFn$rule$type2 = validateFn[rule.type]).call.apply(_validateFn$rule$type2, [_this, val].concat(params)); //  If check fails (not valid && not not | not && valid)
    ...
    value: function extendMulti(obj) {
      ...
      if (!_is["default"].Object(obj)) return; //  For each key in object, check if its value is a function
