...
function vEmail(val) {
  ...
  if (!/^(")?[A-Z0-9&._%+-]+(")?$/gi.test(parts[0])) return !1;
  if (parts[0].charAt(0) === '.') return !1;
  if (parts[0].charAt(parts[0].length - 1) === '.') return !1;
  if (/[.]{2,}/.test(parts[0])) return !1;
  if (parts[1].length > 253) return !1;
  if (/^(?:(?=[a-z0-9-]{1,63}\.)[a-z0-9](?:[a-z0-9-]*[a-z0-9])?\.)+(?=[a-z0-9-]{1,63}z)[a-z0-9](?:[a-z0-9-]*[a-z0-9])?$/ig.test(parts[1])) return !0;
  if (!/^(\w{1,})([.-]?\w)*(\.\w{2,63})+$/.test(parts[1])) return !1;
  return !0;
}